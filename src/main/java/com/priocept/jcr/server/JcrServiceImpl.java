...
//	}
    ...
    public List<Map<String, String>> getNodeTypeIcons() throws SerializedException {
        ...
        for (Map.Entry<Object, Object> propertyPair : properties.entrySet()) {
            tempMap = new HashMap<>();
    ...
    public List<Map<String, List<JcrNode>>> getNodeTree(String path) throws SerializedException {
        ...
        try {
            ...
            for (final String aPathSplit : pathSplit) {
                treeAssociationMap = new HashMap<>();
                pathBuilder.append(aPathSplit.trim()).append("/");
        ...
        try {
            ...
            for (int i = 0; i < nodeIterator.getSize(); i++) {
                ...
                if (node.getPath().contains("[") && node.getPath().contains("]")) {
                    String nameWithBrackets = node.getPath().substring(node.getPath().lastIndexOf('/') + 1, node.getPath().length());
                    newJcrNode = new JcrNode(nameWithBrackets, node.getPath(), node.getPrimaryNodeType()
                            .getName(), properties);
                } else {
                    newJcrNode = new JcrNode(node.getName(), node.getPath(), node.getPrimaryNodeType()
                            .getName(), properties);
                }
    ...
    public List<String> getAvailableNodeTypes() throws SerializedException {
        ...
        try {
            availableNodeTypeList = new ArrayList<>();
...
//		return "Successfully saved. " + sourcePath;
    ...
    public String saveProperties(String sourcePath, JcrNode jcrNode) throws SerializedException {
        ...
        try {
            ...
            Node pathNode = (Node) item;
            for (Map.Entry<String, String> propertyPair : jcrNode.getProperties().entrySet()) {
...
//	@SuppressWarnings("deprecation")
    ...
    public List<RemoteFile> getPossibleIconPaths(String path) throws SerializedException {
        ...
        int charactersToRemove = (rootOfPath + DEFAULT_ICON_PATH_PREFIX + File.separator).length();
